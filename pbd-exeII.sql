...
DECLARE 
    ...
    altura_Juca NUMBER DEFAULT 1.00;
...
BEGIN
    WHILE altura_Juca <= altura_Chico LOOP
        ...
        dbms_output.put_line('Quantidade de anos: '||qtde_anos||' Altura de Chico:  '||ROUND(altura_chico,2)||' Altura de Juca: '||ROUND(altura_juca,2));
    END LOOP;
...
BEGIN
    ...
    ELSE
        WHILE x > 0 LOOP
            ...
            x := x-1;
        END LOOP;
        dbms_output.put_line('O Fatoriall de '||&in_n||'! é: '||fat);
    END IF;
...
ACCEPT in_x NUMBER PROMPT 'INSIRA UM NUMERO'
DECLARE 
    n NUMBER;
    x NUMBER;
    y NUMBER;
...
--3) modelo relacional para entrega
